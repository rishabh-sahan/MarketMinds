...
app = typer.Typer(
    name="MarketMinds",
    help="MarketMinds CLI: Multi-Agents LLM Financial Trading Framework",
...
def update_display(layout, spinner_text=None, stats_handler=None, start_time=None):
    ...
    layout["header"].update(
        Panel(
            "[bold green]Welcome to MarketMinds CLI[/bold green]",
            title="Welcome to MarketMinds",
            border_style="green",
            padding=(1, 2),
            expand=True,
        )
...
def get_user_selections():
    ...
    welcome_content += "[bold green]MarketMinds: Multi-Agents LLM Financial Trading Framework - CLI[/bold green]\n\n"
...
def save_report_to_disk(final_state, ticker: str, save_path: Path):
    """Save the complete analysis as a single PDF.

    Previously this wrote a tree of per-section markdown files plus a
    consolidated one. A PDF is what a reader actually wants: one file, styled,
    with the rating on the cover and every agent's report in order. The
    markdown the agents produce is still the source — it is rendered here, not
    replaced.
    """
    ...
    sections = [
        ...
        ("Trader \u2014 Transaction Proposal", final_state.get("trader_investment_plan", "")),
        ...
        ("Portfolio Manager \u2014 Final Decision",
         risk.get("judge_decision", "") or final_state.get("final_trade_decision", "")),
    ]
    ...
    trade_date = str(final_state.get("trade_date") or datetime.datetime.now().date())
    ...
    pdf = build_run_pdf(
        ...
        meta={
            ...
            "rounds": (
                f"{config.get('max_debate_rounds', '?')} bull/bear \u00b7 "
                f"{config.get('max_risk_discuss_rounds', '?')} risk"
            ),
            "generated": datetime.datetime.now().strftime("%d %b %Y, %H:%M"),
        },
    )

    out = save_path / run_pdf_filename(ticker, trade_date)
    out.write_bytes(pdf)
    return out
...
def analyze(
    ...
    if clear_checkpoints:
        from marketminds.graph.checkpointer import clear_all_checkpoints
